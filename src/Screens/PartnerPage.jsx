import React, { useContext, useEffect } from 'react'
import ITINF from '../Assets/Pagescreens/Asset 5@4x.png'
import { Loadingcontext } from '../Context/LoadingProvider'
import LoadingPage from '../Context/LoadingPage'
import azureLogo from '../Assets/AzureLogo.png'





const PartnerPage = ({HeroTitle, HeroText, HeroImg, DescriptionTitle,textone,texttwo,textthree,textfoor,textfive,textsix,textseven }) => {
    const {Loading,setLoading} = useContext(Loadingcontext)

  useEffect(()=>{
        setLoading(true)
    setTimeout(()=>{
      setLoading(false)
    },1700)

  },[HeroTitle])




  return (<div>
    {Loading === true && <LoadingPage/> }
    <div className="hero w-full h-full flex justify-center items-center pl-[140px] sm:pl-0">
    <div className=' w-full h-full rounded-[40px] flex justify-center items-center pt-16 ' style={{background:'linear-gradient(to right, #EEEBEB, white)'}} >
        <div className="mt-[40px] object-contain ">
            { HeroTitle  && <h1 className='font-extrabold text-[60px] xl:text-[35px] pl-2 pr-20 md:text-[25px] sm:text-[18px] md:pr-5'>Partners</h1>}
        </div>
        <div className="div image pt-6 mb-10">
         { HeroImg &&  <img src={HeroImg} alt="" width={450}/>}
        </div>
    </div>
</div>

<div className="productDesc w-full h-full p-10 mt-10  m-auto rounded-b-[170px] flex justify-center gap-12 flex-wrap">

    <div className={`vendorItem flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
        <div className='w-[80%] h-full bg-no-repeat bg-center bg-contain bg-[url("data:image/png;base64,")] '> </div>
    </div>
        <div className={`vendorItem flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
    <div className=' vendorItem1 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={`flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
          <div className=' vendorItem2 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
         <div className=' vendorItem3 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg `} >
         <div className=' vendorItem4 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
          <div className=' vendorItem5 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg `} >
         <div className=' vendorItem6 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem7 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem8 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem9 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg `} >
            <div className=' vendorItem10 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
        <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem11 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem12 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem13 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem14 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem15 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem16 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem17 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem18 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem19 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem20 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem21 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem22 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem23 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem24 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem25 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem26 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem27 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem28 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem29 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem30 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem31 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem32 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem33 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem34 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem35 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem36 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem37 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    <div className={` flex object-contain justify-center items-center p-4 bg-[white] max-w-1/3 w-[410px] h-[203px] rounded-[38px] shadow-lg  `} >
            <div className=' vendorItem38 w-[80%] h-full bg-no-repeat bg-center bg-contain'> </div>
    </div>
    
</div>

</div>
  )
}

export default PartnerPage